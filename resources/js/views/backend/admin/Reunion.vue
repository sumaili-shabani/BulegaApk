<template>
                ...
                <v-card-text>
                    ...
                    <v-layout row wrap>
                        ...
                        <v-flex xs12 sm12 md12 lg12>
                            <div class="mr-1">
                                <v-autocomplete label="Liste des utilisateurs" prepend-inner-icon="person"
                                    :rules="[(v) => !!v || 'Ce champ est requis']" :items="userListAll"
                                    item-text="name" item-value="id" outlined clearable v-model="svData.user_id"
                        ...
                        <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                                ...
                                <v-text-field
                                    type="time"
                                    label="Le temps démarage de la reunion"
                                    prepend-inner-icon="event"
                                    :rules="[(v) => !!v || 'Ce champ est requis']"
                                    outlined
                                    v-model="svData.hours"
                                ></v-text-field>
                        ...
                        <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                                <v-text-field
                                    type="date"
                                    label="date debit réunion"
                                    prepend-inner-icon="event"
                                    :rules="[(v) => !!v || 'Ce champ est requis']"
                                    outlined
                                    v-model="svData.start_date"
                                    ></v-text-field>
                            </div>
                        ...
                        <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                                <v-text-field
                                    type="date"
                                    label="date  fin réunion"
                                    prepend-inner-icon="event"
                                    :rules="[(v) => !!v || 'Ce champ est requis']"
                                    outlined
                                    v-model="svData.end_date"
                                    ></v-text-field>
                            </div>
                ...
                <template v-slot:default>
                  <thead>
                    <tr>
                        ...
                        <th class="text-left">Nom de la personne </th>
                        <th class="text-left">Nom de la réunion</th>
                        <th class="text-left">Date debit et fin</th>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout réunion ";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de réunion ";
        } else {
          this.titleComponent = "Ajout réunion ";
        }
      },
      ...
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_reunion?page=`);
      },
  
      validate() {
        if (this.$refs.form.validate()) {
          if (this.svData.start_date <= this.svData.end_date) {
                this.isLoading(true);
    
                this.insertOrUpdate(
                  `${this.apiBaseURL}/insert_reunion`,
            ...
            this.showError("La date de debit doit être supérieur à la date fin!");
          }
        }
      },
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_reunion/${id}`).then(
          ({ data }) => {
