          ...
          <v-card :loading="loading">
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                ...
                <v-text-field
                  label="Nom de du groupement ou clan"
                  prepend-inner-icon="extension"
                  :rules="[(v) => !!v || 'Ce champ est requis']"
                  outlined
                  v-model="svData.nomGroup"
                ></v-text-field>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
          ...
          <v-card-text>
            <v-simple-table>
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">Nom du territoire</th>
                    <th class="text-left">Nom de la chefferie</th>
                    <th class="text-left">Nom du groupement</th>
...
export default {
  ...
  computed: {
    ...mapGetters(["roleList","OurTerritoire", "isloading"]),
  },
  methods: {
    ...mapActions(["getRole","getOurTerritoire"]),

    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout chefferie ";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de chefferie ";
      } else {
        this.titleComponent = "Ajout de la chefferie ";
      }
    },
    ...
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_groupement?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_groupement`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_groupement/${id}`).then(
        ({ data }) => {
          var donnees = data.data;

          donnees.map((item) => {
            this.getChefferiTug(item.idTer);
            this.titleComponent = "modification de " + item.nomGroup;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_groupement/${id}`).then(
          ({ data }) => {
